...
def main(city):
    ...
    Path('./osadki_tables').mkdir(exist_ok=True)
    from serverosadki import main
    from factosadki import main_fact
    ...
    if city == 'all':
        ...
        for city in cc:
            for _, prefix in servers:
                fname = f'./osadki_tables/{prefix}_osadkiday{city}.csv'
        ...
        for prefix, df_list in all_dfs.items():
            if df_list:
                ...
                # Сохраняем без колонки 'city', если она не нужна для osadki_graph
                # (можно оставить, osadki_graph её проигнорирует)
                out_file = f'./osadki_tables/{prefix}_osadkidayall.csv'
